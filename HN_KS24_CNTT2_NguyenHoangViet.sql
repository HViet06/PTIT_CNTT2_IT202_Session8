...
SELECT room_type, price_per_day FROM rooms ORDER BY price_per_day;
...
SELECT guest.guest_name AS ten_khach_hang, room.room_type AS loai_phong, checkin.check_in AS ngay_nhan_phong
FROM bookings checkin JOIN guests guest ON checkin.guest_id=guest.guest_id JOIN rooms room ON checkin.room_id=room.room_id;


-- Cho biết mỗi khách đã đặt phòng bao nhiêu lần
SELECT g.guest_name AS ten_khach_hang, COUNT(b.booking_id) AS so_lan_dat_phong FROM guests g
